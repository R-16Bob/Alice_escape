...
{
	//定义部分
	SDL_Surface *screen,*text,*text2;

	//图像指针，image2代表人物，image代表怪物
	//image3 剧情
	SDL_Surface *image2,*image,*image3,*title;
	...
	Uint32 Tstart,Tstop,begin,end;  //时间开始和结束

	//目标矩形，分别代表怪物、人物、文字

	SDL_Rect dRect,dRect2,dRect3,dRect4;
	...
	//显示过场动画和标题
	image3=SDL_LoadBMP("story.bmp");  //加载剧情图片
	title=SDL_LoadBMP("title.bmp");  //加载剧情图片
	if(image3==NULL||title==NULL){
	...
	if(SDL_BlitSurface(image3,NULL,screen,NULL)<0){
	...
	//计算游戏时间
	gcvt((end-begin)/1000,2,str2);
	strcat(str1,str2);
	strcat(str1,str3);
	text=TTF_RenderUTF8_Blended(Nfont,"GAME OVER",red);
	text2=TTF_RenderUTF8_Blended(Nfont,str1,red);
	...
	dRect4.x=200; /*在点（300，300处开始写）*/

	dRect4.y=400;

	dRect4.w=text2->w; /*目标矩形的宽和高分比的是所写字的宽和高*/

	dRect4.h=text2->h;

	SDL_BlitSurface(text2,NULL,screen,&dRect4); /*把目标快速转化*/
	SDL_UpdateRect(screen,0,0,0,0);
